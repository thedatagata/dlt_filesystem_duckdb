...
# fill pond with cleaned swamp water aka source sessions and source pageviews 
def fill_data_pond(df, filepath):
    table = pa.Table.from_pandas(df)
    writer = None
    try:
        # Try to open an existing Parquet file
        writer = pq.ParquetWriter(filepath, table.schema)
        writer.write_table(table=table)
    except Exception as e:
        # If it doesn't exist, create a new Parquet file
        pq.write_table(table, filepath)
    finally:
        if writer:
            writer.close()

def stir_data_pond(filepath):
    # Read the dataset into a single PyArrow Table
...
for swamp_water_bucket in glob.glob("../data_swamp/*.csv"):
    print('Grabbing swamp water from: ' + swamp_water_bucket + '...')
    ...
    print('Filling data pond with sessions...')
    fill_data_pond(sessions_df, '../data_pond/sessions.parquet')
    print('Filling data pond with pageviews...')
    fill_data_pond(pageviews_df, '../data_pond/pageviews.parquet')
    print('Stirring data pond sessions...')
    stir_data_pond('../data_pond/sessions.parquet')
    print('Stirring data pond pageviews...')
    stir_data_pond('../data_pond/pageviews.parquet')
    print('Done!')